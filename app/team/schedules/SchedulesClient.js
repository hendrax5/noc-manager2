...
export default function SchedulesClient({ initialShiftTypes, users, locations, currentUser }) {
  const router = useRouter();
  const [activeTab, setActiveTab] = useState("calendar");
  const [shiftTypes, setShiftTypes] = useState(initialShiftTypes);
  
  const isAdminOrManager = currentUser.permissions?.includes('manage_schedules') || currentUser.role === 'Admin' || currentUser.role === 'Manager';
  ...
  const handleGenerate = async () => {
    ...
    const res = await fetch('/api/schedules/generate', {
      ...
      body: JSON.stringify({ startDate: start, endDate: end, locationId: calLocation || null })
    });
                  ...
                  {Object.values(gridData).map(row => (
                    <tr key={row.user.id}>
                      ...
                      {dayHeaders.map(d => {
                        ...
                        return (
                          <td key={d} style={{ padding: '0.5rem', border: '1px solid #cbd5e1', fontSize: '0.75rem', background: isOff ? '#f8fafc' : isPending ? 'white' : '#dbeafe' }}>
