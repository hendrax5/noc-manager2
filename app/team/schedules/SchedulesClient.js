...
export default function SchedulesClient({
  ...
  const handleGenerate = async () => {
    if (!isScheduleAdmin) return toast.error("Forbidden");
    if (
      !confirm(
  ...
  const handleCellClick = async (userId, day, meta = {}) => {
    ...
    if (swapMode) {
      ...
      const data = await res.json();
      if (!res.ok) toast.error(data.error || "Swap gagal");
      else toast.success("Swap berhasil");
      setSwapA(null);
      await fetchSchedules();
      return;
    }
  ...
  const saveAutoConfig = async () => {
    ...
    if (res.ok) toast.success("Auto-generate settings saved");
  ...
  const forceAutoGenerate = async () => {
    ...
    if (!res.ok) toast.error(data.error || "Failed");
      ...
      setLastKey(data.key || "");
      toast.success(`Auto-generate ${data.key || "done"}`, { duration: 5000 });
    }
  };
