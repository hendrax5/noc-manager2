...
export default function ArticleDetailClient({ article, session, categories }) {
  ...
  const canEdit = session.user.role === 'Admin' || session.user.role === 'Manager' || session.user.permissions?.includes('manage_knowledge') || session.user.id === article.authorId;
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border-color)', paddingBottom: '1.5rem' }}>
          ...
          {canEdit && (
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              ...
              {(session.user.role === 'Admin' || session.user.role === 'Manager' || session.user.permissions?.includes('manage_knowledge')) && (
