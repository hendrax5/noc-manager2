...
export async function DELETE(request, { params }) {
  try {
    const session = await getServerSession(authOptions);
    if (!session || (session.user.role !== 'Admin' && session.user.role !== 'Manager' && !session.user.permissions?.includes('manage_assets'))) {
...
export async function PATCH(request, { params }) {
  try {
    const session = await getServerSession(authOptions);
    if (!session || (session.user.role !== 'Admin' && session.user.role !== 'Manager' && !session.user.permissions?.includes('manage_assets'))) return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
