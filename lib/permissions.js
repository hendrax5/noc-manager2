...
export async function resolvePermissions(userId) {
  ...
  // Auto-grant all permissions if role is Superadmin or Admin
  if (user.role?.name && ['superadmin', 'admin'].includes(user.role.name.toLowerCase())) {
    Object.values(PERMISSIONS).forEach(k => permSet.add(k));
  }
