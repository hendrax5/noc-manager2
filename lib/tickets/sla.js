...
export function getSlaPolicy(configOverride = null) {
  let config = configOverride;
  // getAppConfig() is async in current codebase — never treat a Promise as config
...
export function getPrioritySla(priority = "Medium", configOverride = null) {
  const policy = getSlaPolicy(configOverride);
...
export function buildSlaDeadlines({ priority, enableSla, slaTimerMins, from = new Date(), config = null }) {
  if (!enableSla) {
    return {
      enableSla: false,
  ...
  const policy = getSlaPolicy(config);
  const row = getPrioritySla(priority, config);
