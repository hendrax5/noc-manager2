...
/**
 * @param {"week"|"month"|"custom"} period
 * @param {string|null} anchor — YYYY-MM-DD inside the desired period
 * @param {string|null} startYmd — custom start YYYY-MM-DD
 * @param {string|null} endYmd — custom end YYYY-MM-DD
 */
export function resolveOpsPeriod(period, anchor = null, startYmd = null, endYmd = null) {
  if (period === "custom" && startYmd && endYmd) {
    return {
      period: "custom",
      start: startOfDay(new Date(`${startYmd}T12:00:00`)),
      end: endOfDay(new Date(`${endYmd}T12:00:00`)),
      startDate: startYmd,
      endDate: endYmd,
    };
  }
