...
### Changed
- **Gemini AI API Key Configuration**: Removed the Gemini API Key input field from the NOC Member Comparison Modal. The API Key configuration is now securely centralized in the Admin Settings (`/settings` -> Preferences tab -> Application Branding) and injected dynamically into the analysis route.
...
### Added
- **Ticket Reopen Cycle Timer**: Reset timer functionality when reopened from a resolved state using a metadata-based cycle age (`reopenedAt`) stored in `customData` without losing the original ticket `createdAt` history. This preserves audit integrity, aligns comment timestamps chronologically, and updates workload grids, LiveOps boards, and TTR analytics.
...
### Added
- **Sky View & Tab Switcher**: Refactored the main dashboard (`/dashboard`) into a stateful client wrapper `<DashboardClient />` featuring an elegant Tab Switcher for Admin, Manager, and Superadmin roles.
  - **My Workspace**: Focuses on personal workflows including individual shifts, assigned tickets, and local metrics.
  - **Sky View**: Renders global operational analytics:
